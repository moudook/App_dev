...
class ToolExampleStore {
     ...
     */
    fun getRelevantExamples(query: String, maxExamples: Int = 5): String {
        val lowerQuery = query.lowercase()
        val queryWords = lowerQuery.split(" ", ",", ".", "!", "?").filter { it.length > 2 }.toSet()

        // Score each example by keyword overlap
        val scored = examples.map { example ->
            val matchCount = example.keywords.count { keyword ->
        ...
        val finalExamples = if (relevant.isEmpty()) {
        ...
        return buildString {
            ...
            append("Study these examples carefully. When the user's request matches a pattern, call the corresponding tool IMMEDIATELY without narrating your plan.\n\n")
