...
class VaultRepository(
    ...
    suspend fun get(userId: String): E2EVaultData? = withContext(Dispatchers.IO) {
    ...
    suspend fun store(
        ...
        dataSource.connection.use { conn ->
            ...
            if (exists) {
                val updateStmt = conn.prepareStatement("UPDATE user_vaults SET encrypted_blob = ?, version = ?, updated_at = ? WHERE user_id = ?")
                ...
                val insertStmt = conn.prepareStatement("INSERT INTO user_vaults (user_id, encrypted_blob, version, updated_at) VALUES (?, ?, ?, ?)")
