...
class GoogleDriveService(
    ...
    private fun getAccessToken(): String? {
        ...
        try {
            val stream: InputStream = if (serviceAccountJsonPath != null && java.io.File(serviceAccountJsonPath).exists()) {
     ...
     */
    suspend fun generateUploadUrl(fileName: String, mimeType: String, folderId: String? = null): String? {
        ...
        try {
            val response = httpClient.post("https://www.googleapis.com/upload/drive/v3/files?uploadType=resumable") {
            ...
            if (response.status.isSuccess()) {
                ...
                logger.error("Failed to create resumable upload: ${response.status} - ${response.bodyAsText()}")
                return null
            }
        } catch (e: Exception) {
