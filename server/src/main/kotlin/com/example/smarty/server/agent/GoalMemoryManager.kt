...
class GoalMemoryManager(
    ...
    fun markStepCompleted(description: String, toolUsed: String?, result: String?) {
        val now = Instant.now().toString()
        val completedSteps = progressData.steps.map { step ->
    ...
    fun addStep(description: String, toolUsed: String? = null, result: String? = null, status: String = "in_progress") {
        val now = Instant.now().toString()
        val newStep = ProgressStep(
    ...
    fun addKeyInsight(insight: String) {
        val now = Instant.now().toString()
        memoryData = memoryData.copy(
            keyInsights = memoryData.keyInsights + KeyInsight(insight, now),
            updatedAt = now
        )
    }

    fun addGatheredData(key: String, value: String) {
        val now = Instant.now().toString()
        memoryData = memoryData.copy(
    ...
    fun addImportantResult(stepId: Int, data: String) {
        val now = Instant.now().toString()
        memoryData = memoryData.copy(
    ...
    fun getProgressContext(): String {
        ...
        val completedStr = if (completedSteps.isNotEmpty()) {
            completedSteps.joinToString(" ") { "[✓] ${it.description}" }
        } else {
            "None yet"
        }

        val pendingStr = if (pendingSteps.isNotEmpty()) {
            pendingSteps.joinToString(" ") { "[ ] ${it.description}" }
        } else {
            "None"
        }

        val errorsStr = if (progressData.errors.isNotEmpty()) {
            progressData.errors.joinToString("; ")
        } else {
            "None"
        }

        val insightsStr = if (memoryData.keyInsights.isNotEmpty()) {
