...
class AuthResult private constructor(
    ...
    companion object {
        ...
        ): AuthResult = AuthResult(success = true, userId = userId, user = user)

        fun failure(error: String): AuthResult = AuthResult(success = false, error = error)
    }
 ...
 */
suspend fun ApplicationCall.authenticateUser(respondUnauthorized: Boolean = true): AuthResult {
...
data class ErrorResponse(
    ...
    companion object {
        fun internalError(message: String): ErrorResponse = ErrorResponse(error = message, code = "INTERNAL_ERROR")

        fun badRequest(message: String): ErrorResponse = ErrorResponse(error = message, code = "BAD_REQUEST")

        fun unauthorized(message: String): ErrorResponse = ErrorResponse(error = message, code = "UNAUTHORIZED")

        fun notFound(message: String): ErrorResponse = ErrorResponse(error = message, code = "NOT_FOUND")
    }
}
