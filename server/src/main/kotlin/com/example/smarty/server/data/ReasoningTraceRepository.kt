...
class ReasoningTraceRepository(private val dataSource: javax.sql.DataSource) {
     ...
     */
    suspend fun saveTrace(trace: ReasoningTrace): String =
     ...
     */
    suspend fun saveTraces(traces: List<ReasoningTrace>): Int =
        withContext(Dispatchers.IO) {
            dataSource.connection.use { conn ->
     ...
     */
    suspend fun saveSummary(summary: ReasoningSummary): String =
        withContext(Dispatchers.IO) {
     ...
     */
    suspend fun markTraceAsRevised(
        traceId: String,
     ...
     */
    suspend fun deleteOldTraces(daysOld: Int = 30): Int =
        withContext(Dispatchers.IO) {
            dataSource.connection.use { conn ->
                val sql =
                    """
