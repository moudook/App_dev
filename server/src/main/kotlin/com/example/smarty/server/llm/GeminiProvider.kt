...
class GeminiProvider(
    ...
    override suspend fun generate(
        ...
        try {
            val response: GeminiResponse =
                client.post("$baseUrl/$modelName:generateContent?key=$apiKey") {
    ...
    private fun List<LlmMessage>.toGeminiRequest(tools: List<ToolDefinition>): GeminiRequest {
        val contents =
            this.filter { it.role != LlmMessage.Role.SYSTEM }.map { msg ->
                val role =
                    when (msg.role) {
        ...
        val systemInstruction =
            this.find { it.role == LlmMessage.Role.SYSTEM }?.let {
