        ...
        withContext(Dispatchers.IO) {
            ...
            try {
                ...
                conn.prepareStatement(sql).use { stmt ->
                    ...
                    stmt.setObject(idx++, info.categoryId?.let { runCatching { UUID.fromString(it) }.getOrNull() })
                    stmt.setObject(idx++, info.stackId?.let { runCatching { UUID.fromString(it) }.getOrNull() })
                    stmt.setObject(idx++, info.parentNoteId?.let { runCatching { UUID.fromString(it) }.getOrNull() })
