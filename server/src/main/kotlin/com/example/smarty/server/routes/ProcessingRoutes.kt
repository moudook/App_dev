                ...
                try {
                    ...
                    val result: Any = when {
                        contentType?.startsWith("application/pdf") == true -> {
                            ...
                            if (analysisType == "document") contentAnalysisService.analyzeDocument(pdfResult.text, fileName) else pdfResult
                        }
                        ...
                        contentType?.startsWith("text/") == true -> {
                            ...
                            call.respond(HttpStatusCode.UnsupportedMediaType, mapOf("error" to "Unsupported content type: $contentType"))
                            return@post
                        }
                    }
                    call.respond(HttpStatusCode.OK, result)
            ...
            post("/api/v1/image/direct") {
                ...
                try {
                    ...
                    try {
                        ...
                        supabaseUrl = withContext(Dispatchers.IO) {
                            kreaTool.uploadToSupabase(kreaImageUrl, jobId, com.example.smarty.server.factory.SupabaseClientFactory.getImageBucketName())
                        }
                        if (supabaseUrl != null) log.info("✅ Image uploaded to Supabase: $supabaseUrl")
                        else log.warn("⚠️  Supabase upload returned null - will use Krea URL")
                    } catch (e: Exception) {
                    ...
                    log.info("═".repeat(60))
                    call.respond(HttpStatusCode.OK, ImageGenerationSuccessResponse("image", finalImageUrl, imageSource, request.prompt, jobId))
                    ...
                    withContext(Dispatchers.IO) {
                        callRef.respond(HttpStatusCode.ServiceUnavailable, ImageGenerationSuccessResponse("error", "", "error", "", "", "Image generation service is not configured. KREA_API_KEY is missing from server environment."))
                    }
                } catch (e: Exception) {
                    val elapsed = System.currentTimeMillis() - startTime
                    log.error("❌ /api/v1/image/direct - FAILED after ${elapsed}ms")
                    ...
                    val callRef = call
                    withContext(Dispatchers.IO) {
                        callRef.respond(HttpStatusCode.InternalServerError, ImageGenerationSuccessResponse("error", "", "error", "", "", e.message))
                    }
                }
            }
        }
