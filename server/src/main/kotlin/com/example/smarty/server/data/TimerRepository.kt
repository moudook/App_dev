...
class TimerRepository(private val dataSource: DataSource) {
     ...
     */
    suspend fun listActive(userId: String): List<TimerInfo> = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun deactivate(userId: String, timerId: String): Boolean = withContext(Dispatchers.IO) {
        dataSource.connection.use { conn ->
     ...
     */
    suspend fun delete(userId: String, timerId: String): Boolean = withContext(Dispatchers.IO) {
        dataSource.connection.use { conn ->
