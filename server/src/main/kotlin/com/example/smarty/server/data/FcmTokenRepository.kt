...
class FcmTokenRepository(private val dataSource: DataSource) {
    ...
    suspend fun deleteToken(userId: String, token: String): Boolean = withContext(Dispatchers.IO) {
        dataSource.connection.use { conn ->
    ...
    suspend fun getTokensForUser(userId: String): List<String> = withContext(Dispatchers.IO) {
