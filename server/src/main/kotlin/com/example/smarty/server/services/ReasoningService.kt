...
class ReasoningService(private val repository: ReasoningTraceRepository) {
     ...
     */
    suspend fun logReasoningSteps(traces: List<ReasoningTrace>): Int = withContext(Dispatchers.Default) {
     ...
     */
    suspend fun getReasoningTraces(sessionId: String, messageId: String? = null): List<ReasoningTrace> {
     ...
     */
    suspend fun getReasoningSummary(sessionId: String, messageId: String? = null): ReasoningSummary? {
     ...
     */
    suspend fun markReasoningRevised(traceId: String, revisedByTraceId: String) {
    ...
    fun generateProgressiveDisclosure(traces: List<ReasoningTrace>): ReasoningDisclosureLevels {
        ...
        val briefSteps = finalTraces
            .filter { it.importanceScore > 0.7 }
            .take(5)
            .map { "- ${it.title}" }
        
        // Detailed: All final steps
        val detailedSteps = finalTraces.map { trace ->
            "### ${trace.title}\n${trace.content}"
        }
