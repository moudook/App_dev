...
class ChatFeatureManager(
    ...
    private fun validateCommand(command: AgentCommand): CommandValidationResult {
        return when (command) {
            ...
            is AgentCommand.RetrieveMemories -> {
                ...
                when {
                    queryVal != null && queryVal.isBlank() -> CommandValidationResult.Invalid("query cannot be blank if provided", "query")
                    command.limit <= 0 -> CommandValidationResult.Invalid("limit must be positive", "limit")
                    else -> CommandValidationResult.Valid
                }
            }
            ...
            is AgentCommand.PlanStep -> {
                when {
                    command.stepId.isBlank() -> CommandValidationResult.Invalid("stepId cannot be blank", "stepId")
                    command.description.isBlank() -> CommandValidationResult.Invalid("description cannot be blank", "description")
                    else -> CommandValidationResult.Valid
                }
            }
    ...
    private fun summarizeCommand(command: AgentCommand): String = when (command) {
        // Knowledge & Note operations - content lengths only
        is AgentCommand.CaptureKnowledge -> "title.len=${command.title.length} | content.len=${command.content.length} | source.len=${command.source.length}"
        is AgentCommand.SearchKnowledge -> "query.len=${command.query.length}"
        ...
        // Memory operations
        is AgentCommand.StoreMemory -> "content.len=${command.content.length} | hasScope=${command.scope != null}"
        is AgentCommand.UpdateMemory -> "id=${command.id} | content.len=${command.content.length} | hasType=${command.type != null}"
        is AgentCommand.DeleteMemory -> "id=${command.id}"
        is AgentCommand.RetrieveMemories -> "hasQuery=${command.query != null} | limit=${command.limit}"
        ...
        is AgentCommand.PlanStep -> "stepId=${command.stepId}"
    ...
    private val agentEventSink = object : AgentEventSink {
        ...
        override fun emit(command: AgentCommand) {
            ...
            when (command) {
                ...
                is AgentCommand.PlanStep -> {
                    onPlanStatusChanged(command.description)
                }
    ...
    private val clientCommandExecutor = object : ClientCommandExecutor {
        ...
        override suspend fun searchKnowledge(query: String, filter: String?): List<RecallResult> {
            // Knowledge search is typically handled by semantic search
            ...
            try {
                ...
                when (commandResult) {
                    is CommandResult.Handled -> {
                        ...
                        val assistantMessage = ChatMessage(id = java.util.UUID.randomUUID().toString(), role = ChatRole.ASSISTANT, content = commandResult.response, timestamp = System.currentTimeMillis())
                    ...
                    is CommandResult.NavigateTo -> {
                        ...
                        val assistantMessage = ChatMessage(id = java.util.UUID.randomUUID().toString(), role = ChatRole.ASSISTANT, content = response, timestamp = System.currentTimeMillis())
                        chatManager.addAssistantMessage(assistantMessage)
                        chatManager.saveMessagePair(
                            userMessage = userMessage,
                            assistantMessage = assistantMessage
                    ...
                    is CommandResult.HandledAndPassToLLM -> {
                        val localMessage = ChatMessage(id = java.util.UUID.randomUUID().toString(), role = ChatRole.ASSISTANT, content = commandResult.response, timestamp = System.currentTimeMillis())
                ...
                chatManager.addAssistantMessage(ChatMessage(id = java.util.UUID.randomUUID().toString(), role = ChatRole.ASSISTANT, content = application.getString(R.string.error_prefix, e.message ?: application.getString(R.string.unknown_error)), timestamp = System.currentTimeMillis()))
            } finally {
