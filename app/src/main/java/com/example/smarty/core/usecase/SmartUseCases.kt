...
class GetNotesWithTagsUseCase @Inject constructor(
...
class CreateNoteWithAutoTaggingUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(note: Note, userId: String): Note {
        return repository.createNoteWithAutoTagging(note, userId)
    }
}

class FindRelatedNotesUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(noteId: String, limit: Int = 10): List<Note> {
        return repository.findRelatedNotes(noteId, limit)
    }
}

class UpdateNoteWithVersionUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
...
class AssignTagToNoteUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        noteId: String,
...
class CreateTaskFromNoteUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        noteId: String,
...
class CompleteTaskUseCase @Inject constructor(
    ...
    suspend operator fun invoke(taskId: String, userId: String) {
...
class RecordReasoningStepUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        sessionId: String,
...
class CreateReasoningSummaryUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        sessionId: String,
        ...
        return repository.createReasoningSummary(
            sessionId, userId, oneLiner, briefSummary, detailedSummary, reasoningType
        )
    }
}

class SaveAgentCheckpointUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        sessionId: String,
        ...
        return repository.saveAgentCheckpoint(
            sessionId, userId, stateJson, contextJson, memoryJson, workflowId, checkpointType
        )
    }
}
...
class ShareItemUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(
        ownerId: String,
...
class GenerateDailyDigestUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    suspend operator fun invoke(userId: String, date: Long): DailyDigestEntity {
...
class StartBackgroundSyncUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
    operator fun invoke() {
...
class GetUserWithSyncStateUseCase @Inject constructor(
    private val repository: SmartRepository,
) {
