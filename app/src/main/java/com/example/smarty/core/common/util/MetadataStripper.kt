...
object MetadataStripper {
    ...
    private val BUFFER_SIZE: Int
        get() = try { ResourceManager.getOptimalBufferSize() } catch (e: Exception) { 32768 }

    private val NIO_BUFFER_SIZE: Int
        get() = try { ResourceManager.getOptimalNioBufferSize() } catch (e: Exception) { 65536 }
    ...
    private val EXIF_TAGS_TO_REMOVE = arrayOf(
    ...
    ): StripResult = withContext(Dispatchers.IO) {
    ...
    private suspend fun stripImageMetadata(
        ...
        try {
            ...
            Log.d(TAG, "Image stripped: ${outputFile.name} in ${processingTime}ms " +
     ...
     */
    private fun streamStripPng(input: InputStream, outputFile: File) {
        ...
        BufferedOutputStream(FileOutputStream(outputFile), BUFFER_SIZE).use { output ->
            ...
            while (bufferedInput.read(chunkHeader) == 8) {
                val length = ((chunkHeader[0].toInt() and 0xFF) shl 24) or
    ...
    private suspend fun reencodeImage(
        ...
        context.contentResolver.openInputStream(sourceUri)?.use { input ->
            ...
            val maxDim = try { ResourceManager.getMaxImageDimension() } catch (e: Exception) { 2048 }
            var sampleSize = 1
    ...
    private fun logVideoMetadata(context: Context, sourceUri: Uri) {
        try {
     ...
     */
    private fun extractAudioTitle(context: Context, sourceUri: Uri): String? {
        return try {
            val retriever = MediaMetadataRetriever()
            try {
                context.contentResolver.openFileDescriptor(sourceUri, "r")?.use { pfd ->
    ...
    private suspend fun fastCopy(
        ...
        val largeThreshold = try { ResourceManager.getLargeFileThreshold() } catch (e: Exception) { 5 * 1024 * 1024L }
