...
object StringUtils {
    ...
    fun capitalizeFirst(text: String): String = text.replaceFirstChar { it.uppercase() }
...
object CollectionUtils {
    fun <T> List<T>.safeGet(
        index: Int,
        default: T,
    ): T = if (index in indices) this[index] else default

    fun <T> List<T>.chunkedSafe(size: Int): List<List<T>> = if (isEmpty()) emptyList() else chunked(size)

    fun <T, R> List<T>.mapNotNullTransform(transform: (T) -> R?): List<R> = mapNotNull { transform(it) }
}
