...
class AdaptiveSemanticSearchEngine {
    ...
    fun analyzeDatabaseContent(notes: List<Note>): SearchConfiguration {
        ...
        val (keywordWeight, semanticWeight, vectorWeight) =
            when (algorithmSelection) {
    ...
    private fun analyzeContentCharacteristics(notes: List<Note>): ContentCharacteristics {
        ...
        for (i in notes.indices) {
            for (j in i + 1 until notes.size) {
                val similarity =
                    SemanticSearchEngine.calculateSimilarity(
    ...
    fun <T> adaptiveSearch(
        ...
        return when (config.algorithmSelection) {
            AlgorithmSelection.KEYWORD_HEAVY ->
                keywordHeavySearch(
                    query,
                    items,
                    idExtractor,
            ...
            AlgorithmSelection.SEMANTIC_HEAVY ->
                semanticHeavySearch(
                    query,
                    items,
                    idExtractor,
            ...
            AlgorithmSelection.VECTOR_HEAVY ->
                vectorHeavySearch(
                    query,
                    items,
                    idExtractor,
    ...
    private fun <T> hybridSearch(
        ...
        val keywordResults =
            keywordHeavySearch(
                ...
                contentExtractor,
                dataType,
                config.copy(algorithmSelection = AlgorithmSelection.KEYWORD_HEAVY),
            )
        val semanticResults =
            semanticHeavySearch(
                query,
                ...
                contentExtractor,
                dataType,
                config.copy(algorithmSelection = AlgorithmSelection.SEMANTIC_HEAVY),
            )
        val vectorResults =
            vectorHeavySearch(
                query,
                ...
                contentExtractor,
                dataType,
                config.copy(algorithmSelection = AlgorithmSelection.VECTOR_HEAVY),
            )
        ...
        val finalResults =
            resultMap.values.map { (score, result, _) ->
