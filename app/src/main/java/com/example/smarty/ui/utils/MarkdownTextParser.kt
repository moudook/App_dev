...
object MarkdownTextParser {
     ...
     */
    fun hasInlineMath(text: String): Boolean = INLINE_MATH_DETECT.containsMatchIn(text)

    /**
     * Check if text contains code blocks.
     */
    fun hasCodeBlock(text: String): Boolean = CODE_FENCE.containsMatchIn(text)
     ...
     */
    fun removeThinkTags(content: String): String = THINK_TAG_REGEX.replace(content, "").trim()

    /**
     * Check if content has partial THINK tag (still streaming).
     */
    fun hasPartialThinkTag(content: String): Boolean = PARTIAL_THINK_REGEX.containsMatchIn(content)
