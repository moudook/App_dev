...
fun LivingOrbVisualizer(
    ...
    Canvas(modifier = modifier.size(size)) {
        ...
        val jellyCoreRadius = (minCoreRadius + (smoothedAmp * 1.2f * (maxCoreRadius - minCoreRadius)))
        ...
        val ghostCoreRadius = (minCoreRadius + (targetAmp * 1.2f * (maxCoreRadius - minCoreRadius)))
            .coerceIn(minCoreRadius, maxCoreRadius)
        
        val ghostInnerAuraRadius = (ghostCoreRadius * 1.5f).coerceAtMost(maxRadius * 0.95f)
        
        // Idle breathing (subtle)
        val idleBreathe = if (isPlaying) {
