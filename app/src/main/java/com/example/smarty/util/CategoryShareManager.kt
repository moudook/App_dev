...
object CategoryShareManager {
    ...
    fun createDeepLink(category: Category, notes: List<Note>): String {
        ...
        return "cogni://import?data=$encoded"
    }
    ...
    fun saveQRCodeToCache(context: Context, bitmap: Bitmap, categoryName: String): Uri? {
        return try {
            ...
            cachePath.mkdirs()
            val fileName = "cogni_${categoryName.replace(" ", "_")}_qr.png"
    ...
    suspend fun shareCategory(
        ...
        val shareText = buildString {
            appendLine("Cogni Category: ${category.name}")
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_SUBJECT, "Cogni: ${category.name}")
