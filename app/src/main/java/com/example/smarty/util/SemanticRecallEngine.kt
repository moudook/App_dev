...
object SemanticRecallEngine {
    ...
    private val recallCache = mutableMapOf<String, List<RecallResult>>()
    ...
    fun semanticRecall(
        ...
        // Check cache first
        recallCache[cacheKey]?.let { cached ->
            Log.d(TAG, "Cache hit for recall: ${cached.size} items")
            return cached.filter { it.combinedScore >= minRelevance }
        }
        
        val results = performRecall(query, context, allNotes, minRelevance)
        ...
        recallCache[cacheKey] = results
        
        return results.filter { it.combinedScore >= minRelevance }
    }
    
    private fun performRecall(
        query: String,
        context: RecallContext,
        allNotes: List<Note>,
        minRelevance: Double
