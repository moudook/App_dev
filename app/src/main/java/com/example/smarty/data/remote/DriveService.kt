...
class DriveService(
    ...
    companion object {
        private const val APP_FOLDER_NAME = "Cogni Backups"
     ...
     */
    suspend fun getOrCreateAppFolder(): Result<String> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun listBackups(): Result<List<BackupMetadata>> = withContext(Dispatchers.IO) {
        try {
            ...
            val folderId = getOrCreateAppFolder().getOrElse {
            ...
            val backups = result.files.mapNotNull { file ->
                try {
                    ...
                    // Format: cogni_backup_YYYYMMDD_HHmmss.zip
    ...
    suspend fun downloadBackup(
        ...
        try {
            ...
            val fileMeta = driveService.files().get(fileId)
                .setFields("size")
                .execute()
     ...
     */
    suspend fun deleteOldBackups(keepCount: Int = DEFAULT_KEEP_BACKUP_COUNT): Result<Int> = withContext(Dispatchers.IO) {
        try {
            val account = authManager.getCurrentAccount()
                ?: return@withContext Result.failure(Exception("Not signed in"))
