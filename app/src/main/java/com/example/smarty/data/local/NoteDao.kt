...
interface NoteDao {
    ...
    @Query(
        "SELECT * FROM notes WHERE isArchived = 0 AND (:query IS NULL OR :query = '' OR title LIKE '%' || :query || '%' OR content LIKE '%' || :query || '%' OR summary LIKE '%' || :query || '%') AND (:hasTypeFilter = 0 OR type IN (:types)) ORDER BY createdAt DESC",
    )
    fun searchNotes(
        query: String?,
    ...
    suspend fun clearCategoryFromNotes(
        categoryId: String,
        timestamp: Long = System.currentTimeMillis(),
    )
    ...
    @SkipQueryVerification
    @Query(
        "SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 ORDER BY createdAt DESC",
    )
    ...
    @SkipQueryVerification
    @Query(
        "SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 AND type IN (:types)",
    )
    ...
    @SkipQueryVerification
    @Query(
        "SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 AND type IN (:types) ORDER BY createdAt DESC",
    )
    suspend fun searchNotesFts4WithType(
        query: String,
        types: List<com.example.smarty.core.domain.model.NoteType>,
    ): List<Note>
    ...
    @Query(
        "SELECT * FROM notes WHERE reminderText IS NOT NULL AND (reminderExpiresAt IS NULL OR reminderExpiresAt > :now) ORDER BY reminderExpiresAt ASC",
    )
    ...
    suspend fun deleteNoteIfServerIsNewer(
        noteId: String,
    ...
    suspend fun updateNoteIfServerIsNewer(
        id: String,
        title: String,
        content: String,
        summary: String?,
        ...
        parentNoteId: String?,
        whySaved: String?,
        processingStatus: com.example.smarty.core.domain.model.ProcessingStatus,
        contentHash: String?,
        processedContentHash: String?,
        isArchived: Boolean,
        isPinned: Boolean,
        isFavorite: Boolean,
        isFullPrivacy: Boolean,
        ...
        tagsJson: String?,
        chunkAnalysesJson: String?,
        reminderText: String?,
        reminderExpiresAt: Long?,
        metadata: String?,
        wordCount: Int?,
        updatedAt: Long,
    ): Int
