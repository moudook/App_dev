...
interface NoteDao {
    ...
    @Query("SELECT * FROM notes WHERE isArchived = 0 AND (:query IS NULL OR :query = '' OR title LIKE '%' || :query || '%' OR content LIKE '%' || :query || '%' OR summary LIKE '%' || :query || '%') AND (:hasTypeFilter = 0 OR type IN (:types)) ORDER BY createdAt DESC")
    ...
    suspend fun updateNoteCategory(noteId: String, categoryId: String, categoryName: String, timestamp: Long = System.currentTimeMillis())
    @Transaction
    ...
    @SkipQueryVerification
    @Query("SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0")
    suspend fun searchNotesFts(query: String): List<Note>
    @SkipQueryVerification
    @Query("SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 ORDER BY createdAt DESC")
    ...
    @SkipQueryVerification
    @Query("SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 AND type IN (:types)")
    suspend fun searchNotesFtsWithType(query: String, types: List<com.example.smarty.core.domain.model.NoteType>): List<Note>
    @SkipQueryVerification
    @Query("SELECT notes.* FROM notes JOIN notes_fts ON notes.rowid = notes_fts.rowid WHERE notes_fts MATCH :query AND isArchived = 0 AND type IN (:types) ORDER BY createdAt DESC")
    ...
    @Query("SELECT COUNT(*) FROM notes")
    suspend fun hasAnyNotes(): Boolean
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertNote(note: Note)
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertNotes(notes: List<Note>)
    @Query("DELETE FROM notes WHERE id = :noteId AND updatedAt < :serverUpdatedAt")
    suspend fun deleteNoteIfServerIsNewer(noteId: String, serverUpdatedAt: Long): Int
    @Query("UPDATE notes SET title = :title, content = :content, summary = :summary, sourceUrl = :sourceUrl, imageUri = :imageUri, fileUri = :fileUri, fileName = :fileName, fileMimeType = :fileMimeType, fileSize = :fileSize, type = :type, categoryId = :categoryId, categoryName = :categoryName, stackId = :stackId, parentNoteId = :parentNoteId, whySaved = :whySaved, processingStatus = :processingStatus, contentHash = :contentHash, processedContentHash = :processedContentHash, isArchived = :isArchived, isPinned = :isPinned, isFavorite = :isFavorite, isFullPrivacy = :isFullPrivacy, excludeFromAiChat = :excludeFromAiChat, isAiCreated = :isAiCreated, isViewed = :isViewed, todoContent = :todoContent, attachmentsJson = :attachmentsJson, tagsJson = :tagsJson, chunkAnalysesJson = :chunkAnalysesJson, reminderText = :reminderText, reminderExpiresAt = :reminderExpiresAt, metadata = :metadata, wordCount = :wordCount, updatedAt = :updatedAt WHERE id = :id AND updatedAt < :updatedAt")
    suspend fun updateNoteIfServerIsNewer(id: String, title: String, content: String, summary: String?, sourceUrl: String?, imageUri: String?, fileUri: String?, fileName: String?, fileMimeType: String?, fileSize: Long?, type: com.example.smarty.core.domain.model.NoteType, categoryId: String?, categoryName: String?, stackId: String?, parentNoteId: String?, whySaved: String?, processingStatus: com.example.smarty.core.domain.model.ProcessingStatus, contentHash: String?, processedContentHash: String?, isArchived: Boolean, isPinned: Boolean, isFavorite: Boolean, isFullPrivacy: Boolean, excludeFromAiChat: Boolean, isAiCreated: Boolean, isViewed: Boolean, todoContent: String?, attachmentsJson: String?, tagsJson: String?, chunkAnalysesJson: String?, reminderText: String?, reminderExpiresAt: Long?, metadata: String?, wordCount: Int?, updatedAt: Long): Int
}