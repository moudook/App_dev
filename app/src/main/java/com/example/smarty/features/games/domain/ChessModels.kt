...
data class Pc(val t: Typ, val c: Col)
data class Sq(val r: Int, val f: Int)   // row 0=top(black backrank), file 0=a

typealias Board = List<Pc?>

fun Board.at(r: Int, f: Int): Pc? = this[r * 8 + f]
fun Board.at(sq: Sq): Pc? = at(sq.r, sq.f)
fun Board.set(sq: Sq, p: Pc?): Board = toMutableList().also { it[sq.r * 8 + sq.f] = p }
fun Board.move(from: Sq, to: Sq): Board = set(from, null).set(to, at(from))

fun ok(r: Int, f: Int) = r in 0..7 && f in 0..7
...
const val PT_P = 0; const val PT_N = 1; const val PT_B = 2
const val PT_R = 3; const val PT_Q = 4; const val PT_K = 5

fun pcBbIdx(pt: Int, col: Int): Int = pt * 2 + col  // 0..11

// Square index helpers (bitboard index = (7-row)*8+file, i.e. a1=0, h8=63)
fun sqIdx(r: Int, f: Int): Int = (7 - r) * 8 + f
...
fun Bb.popLsb(): Pair<Bb, Int> { val i = lsb(); return this xor bit(i) to i }
