...
class SpeechToTextState(
    ...
    fun stopListening() {
        ...
        try {
            ...
            // BUG FIX: Delay resetting mode tracking to ensure the UI has time 
            // to read the mode during result collection from the SharedFlow
            handler.postDelayed({
            ...
            unpauseRunnable = Runnable {
                com.example.smarty.features.voice.VoskWakeWordManager.isGloballyPaused = false
    ...
    private fun startRecognition(languageCode: String?) {
        ...
        com.example.smarty.features.voice.VoskWakeWordManager.isGloballyPaused = true
