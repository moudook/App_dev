...
object CommandValidator {
     ...
     */
    fun validate(command: AgentCommand): Result = when (command) {
        // === NOTE OPERATIONS ===
        is AgentCommand.AddNote -> {
            when {
                command.content.isBlank() -> Result.Invalid("content cannot be blank", "content")
                command.content.length > MAX_CONTENT_LENGTH -> Result.Invalid("content exceeds max length ($MAX_CONTENT_LENGTH)", "content")
                else -> Result.Valid
            }
        }
        ...
        is AgentCommand.ControlAudio -> {
            when {
                ...
                command.action.lowercase() !in ALLOWED_AUDIO_ACTIONS -> Result.Invalid("action must be one of: $ALLOWED_AUDIO_ACTIONS", "action")
                else -> Result.Valid
            }
        }
     ...
     */
    fun summarize(command: AgentCommand): String = when (command) {
