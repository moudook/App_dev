...
class AssistViewModel(
    ...
    private val clientCommandExecutor =
        object : ClientCommandExecutor {
            ...
            override fun addTodoToNote(
                ...
                viewModelScope.launch { noteOperationsManager.addTodoToNote(noteId, text) }
            }
            ...
            override suspend fun getCategoryStats(): List<CategoryStatInfo> =
                noteOperationsManager.getCategoryStats(
                    _categories.value,
                    _notes.value + _archivedNotes.value,
                )
            ...
            override suspend fun advancedSearch(
                ...
                searchFeatureManager.advancedSearch(
                    query,
                    algorithm,
                    limit,
                    minScore,
            ...
            override suspend fun performRecall(
                ...
                searchFeatureManager.performRecall(
                    query,
                    minScore,
                )
            ...
            override suspend fun findMatchingAudio(query: String): AudioFeatureManager.AudioSearchResult =
                audioFeatureManager.findAudioTrack(
                    query,
                )
            ...
            override suspend fun queryCalendarEvents(query: String?): List<CalendarEvent> =
                if (query.isNullOrBlank()) {
                    ...
                    calendarFeatureManager.searchEvents(
                        query,
                    )
                }
    ...
    private suspend fun addSmartyMessage(
    ...
    fun saveAndClose() {
        viewModelScope.launch { chatManager.exitChatMode() }
    }
    ...
    fun clearMessages() {
        viewModelScope.launch { chatManager.createNewChatSession() }
    }

    fun setAssistContext(
        selectedText: String?,
        referringPackage: String,
    ) {
        _assistContext.value = AssistContext(selectedText, referringPackage)
    }
