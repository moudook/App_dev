...
// import com.example.smarty.core.common.util.ImageTextExtractor
...
// import com.example.smarty.core.common.util.PDFChunkedResult
// import com.example.smarty.core.common.util.PDFExtractionResult
// import com.example.smarty.core.common.util.PDFTextExtractor
...
class NoteOperationsManager(
    ...
    // PDF extraction helper removed (Server-side)
    // private val pdfExtractor = PDFTextExtractor(context)

    // Database write batcher for performance (50-300% improvement)
    private val writeBatcher: DatabaseWriteBatcher = DatabaseWriteBatcher(repository, scope).also { batcher ->
        batcher.start()
    }
                    ...
                    noteIds.forEach { id ->
                        ...
                        note?.let {
                            ...
                            it.getAllAttachmentUris().forEach { uri ->
                                if (!isFileInUse(uri)) {
                                    com.example.smarty.core.common.util.FileStorageHelper.deleteFile(context, uri)
                                }
                            }
    ...
    /**
     * Process PDF documents with AI analysis (Deprecated - handled via processNoteWithAi or server)
     * Kept as placeholder or simple wrapper if needed.
     */
    suspend fun processPdfWithAi(note: Note) {
       // Logic moved to server-side via processNoteWithAi handling
       // Just process as normal note
       processNoteWithAi(note)
    }

    /**
     * Helper methods for local PDF processing removed.
     */
        ...
        try {
            ...
            for (attachment in attachments) {
                try {
                    ...
                    when {
                        // IMAGES: Upload to Server for OCR/Analysis
                        ...
                        // PDFs: Use server-side processing
                        mimeType == "application/pdf" -> {
                            Log.d(TAG, "Routing PDF to Server: ${attachment.fileName}")
                            // For PDFs, we can either extract text locally and send to analyzeDocument (current flow)
                            // OR send the PDF bytes to processPdf (server does extraction).
                            // The plan suggests "Remove local PDF/OCR, use server".

                            val fileBytes = context.contentResolver.openInputStream(uri)?.use { it.readBytes() }
                            if (fileBytes != null) {
                                try {
                                    val extractedText = aiService.processPdf(fileBytes, attachment.fileName)
                                    // We can add this text to extractedTexts for later combined analysis
                                    // OR we can trigger document analysis flow if it's the main content.

                                    // If we are in processNoteWithAi, we want to categorize/summarize the note.
                                    // So adding text to extractedTexts works well for mixed content.
                                    ...
                                    // Fallback? No, we are thin client.
                                }
                            }
                            // Skip the recursive processPdfWithAi call if we handle it here
            ...
            val attachmentMetadata = attachments.map { attachment ->
                com.example.smarty.core.domain.model.AttachmentMetadata.fromNoteAttachment(attachment)
     ...
     */
    fun getCategoryStats(categories: List<com.example.smarty.core.domain.model.Category>, activeNotes: List<Note>): List<CategoryStatInfo> {
