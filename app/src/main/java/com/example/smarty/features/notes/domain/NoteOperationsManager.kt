...
class NoteOperationsManager(
    ...
    fun setEngagementManager(manager: NoteEngagementManager) {
        engagementManager = manager
    }

    private fun trackNoteCreation() {
        engagementManager?.onNoteCreated()
    }
    ...
    fun addNote(
        ...
        scope.launch {
            noteOperationMutex.withLock {
                ...
                val category =
                    initialCategory?.let { repository.getOrCreateCategory(it) }
                        ?: repository.getOrCreateCategory(context.getString(com.example.smarty.R.string.category_quick_notes))
                ...
                when {
                    ...
                    sharedContent.text != null -> {
                        ...
                        Note(
                            ...
                            sourceUrl =
                                if (type != NoteType.BRAIN_DUMP &&
                                    sharedContent.text.contains("://")
                                ) {
                                    ContentTypeDetector.extractUrl(sharedContent.text)
                                } else {
                                    null
                                },
                            type = type,
    ...
    fun refreshNotes() {
        scope.launch { repository.refreshNotes() }
    }

    fun syncCategoryCounts() {
        scope.launch { repository.syncAllCategoryCounts() }
    }
                    ...
                    noteIds.forEach { id ->
                        repository.getNoteById(id)?.let { note ->
                            repository.updateNote(
                                note.copy(categoryId = category.id, categoryName = category.name, updatedAt = System.currentTimeMillis()),
                            )
                        }
                    }
                }
    ...
    fun pinNote(noteId: String) {
        scope.launch { repository.pinNote(noteId) }
    }

    fun unpinNote(noteId: String) {
        scope.launch { repository.unpinNote(noteId) }
    }

    fun toggleNotePin(noteId: String) {
        scope.launch { repository.toggleNotePin(noteId) }
    }
    ...
    fun clearNoteReminder(noteId: String) {
        scope.launch { repository.clearNoteReminder(noteId) }
    }
    ...
    suspend fun restoreNoteVersion(
        noteId: String,
        versionId: String,
    ): Boolean = repository.restoreNoteVersion(noteId, versionId)
                        ...
                        if (newAttachments != null) {
                            ...
                            updatedNote =
                                if (primary != null) {
                                    ...
                                    updatedNote.copy(fileUri = null, fileName = null, fileMimeType = null, fileSize = null, imageUri = null)
                                }
                        }
    ...
    fun markNoteAsViewed(noteId: String) {
        scope.launch { repository.updateNoteViewedStatus(noteId, true) }
    }
    ...
    fun updateNoteCategory(
        ...
        scope.launch { repository.updateNoteCategory(noteId, categoryId, categoryName) }
    }
        ...
        try {
                        ...
                        mimeType.startsWith("image/") -> {
                            ...
                            if (fileBytes != null) {
                                try {
                                    ...
                                    extractedTexts.add(
                                        context.getString(
                                            com.example.smarty.R.string.image_content_label,
                                            attachment.fileName,
                                            extractedText,
                                        ),
                                    )
                                    ...
                                    Log.e(TAG, "Server image processing failed: ${e.message}")
                                }
                            }
                        }
                        mimeType == "application/pdf" -> {
                            ...
                            if (fileBytes != null) {
                                try {
                                    ...
                                    extractedTexts.add(
                                        context.getString(
                                            com.example.smarty.R.string.file_content_label,
                                            attachment.fileName,
                                            extractedText,
                                        ),
                                    )
                                } catch (e: Exception) {
                                    Log.e(TAG, "Server PDF processing failed: ${e.message}")
                                }
                            }
                        }
                            ...
                            try {
                                val textContent =
                                    context.contentResolver
                                        .openInputStream(
                                            uri,
                                        )?.use { it.bufferedReader().readText().take(10000) }
                                if (!textContent.isNullOrBlank()) {
                                    extractedTexts.add(
                                        context.getString(com.example.smarty.R.string.file_content_label, attachment.fileName, textContent),
                                    )
            ...
            val enhancedContent =
                if (extractedTexts.isNotEmpty()) {
                    ...
                    context.getString(
                        com.example.smarty.R.string.attachments_label,
            ...
            val attachmentMetadata =
                attachments
                    .map {
                        com.example.smarty.core.domain.model.AttachmentMetadata
                            .fromNoteAttachment(it)
    ...
    suspend fun storeWithoutAnalysis(note: Note) {
        val fallbackResponse =
            com.example.smarty.data.remote.AIResponseParser
                .smartFallbackCategorization(context, note.content)
