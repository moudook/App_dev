...
class ChessEngine {
        ...
        if (pc.t == Typ.K && !isInCheckInternal(board, pc.c, ep)) {
            ...
            if (from == Sq(row, 4)) {
                ...
                if (ksRight &&
                    board.at(row, 5) == null &&
                    board.at(row, 6) == null &&
                    board.at(row, 7) == Pc(Typ.R, pc.c) &&
                    !isAttackedInternal(board, Sq(row, 5), opp(pc.c), ep) &&
                    !isAttackedInternal(board, Sq(row, 6), opp(pc.c), ep)
                ) {
                    moves.add(Sq(row, 6))
                }
                ...
                if (qsRight &&
                    ...
                    board.at(row, 2) == null &&
                    board.at(row, 1) == null &&
                    board.at(row, 0) == Pc(Typ.R, pc.c) &&
                    !isAttackedInternal(board, Sq(row, 3), opp(pc.c), ep) &&
                    !isAttackedInternal(board, Sq(row, 2), opp(pc.c), ep)
                ) {
                    moves.add(Sq(row, 2))
                }
            }
    ...
    fun executeMove(
        ...
        board = res.bd
        ep = res.newEp
        cr = updateRightsInternal(cr, movingPc, from, to, captured)
        lastFrom = from
        lastTo = to
        turn = opp(turn)
        ...
        if (check && !hasMoves) {
            gameOver = true
            status = "Checkmate! $prevName wins!"
    ...
    private fun pseudoMovesInternal(
        bd: Board,
        from: Sq,
        ep: Sq?,
    ): List<Sq> {
        ...
        when (pc.t) {
            Typ.P -> {
                val dir = if (pc.c == Col.W) -1 else 1
                val startRow = if (pc.c == Col.W) 6 else 1
            ...
            Typ.K -> {
                for (dr in -1..1) for (df in -1..1) if (dr != 0 || df != 0) addIfLegal(from.r + dr, from.f + df)
            }
        }
        return res
    }

    private data class MoveRes(
        val bd: Board,
        val newEp: Sq?,
    )
    ...
    private fun applyMoveInternal(
        ...
        var nb = bd
        var newEp: Sq? = null
        if (pc.t == Typ.P && ep != null && to == ep) {
            val dir = if (pc.c == Col.W) -1 else 1
            nb = nb.set(Sq(to.r - dir, to.f), null)
        }
        if (pc.t == Typ.K && kotlin.math.abs(to.f - from.f) == 2) {
            val row = from.r
            if (to.f == 6) {
                nb = nb.move(Sq(row, 7), Sq(row, 5))
            } else if (to.f == 2) {
                nb = nb.move(Sq(row, 0), Sq(row, 3))
            }
        }
        nb = nb.move(from, to)
    ...
    private fun isAttackedInternal(
        bd: Board,
        sq: Sq,
        byCol: Col,
        ep: Sq?,
            ...
            for (f in 0..7) {
                ...
                if (p.c == byCol) {
                    if (p.t ==
                        ...
                        ; if (sq.r == r + dir && (sq.f == f - 1 || sq.f == f + 1)) return true
                        continue
                    }
    ...
    private fun isInCheckInternal(
        bd: Board,
        c: Col,
        ep: Sq?,
    ) = kingSquareInternal(bd, c)?.let { isAttackedInternal(bd, it, opp(c), ep) } ?: false

    private fun kingSquareInternal(
        bd: Board,
        c: Col,
    ): Sq? {
        for (r in 0..7) {
    ...
    private fun anyLegalMoveInternal(
        bd: Board,
        c: Col,
        ep: Sq?,
        cr: CastleRights,
        ...
        for (r in 0..7) {
            for (f in 0..7) {
                ...
                if (p.c ==
                    ...
                    for (to in pseudoMovesInternal(bd, Sq(r, f), ep)) {
                        val (nb, newEp) = applyMoveInternal(bd, Sq(r, f), to, ep)
                        if (!isInCheckInternal(nb, c, newEp)) return true
                    }
                }
            }
        }
    ...
    private fun updateRightsInternal(
        cr: CastleRights,
        pc: Pc,
        from: Sq,
        to: Sq,
        ...
        if (pc.t ==
            ...
            r = if (pc.c == Col.W) r.copy(wKingside = false, wQueenside = false) else r.copy(bKingside = false, bQueenside = false)
        }
        if (pc.t == Typ.R) {
