 ...
 */
/**
 * Power profile for wake word detection.
 * Trade-off between detection range and battery life.
 */
enum class WakeWordPowerProfile {
    /** Minimal CPU usage, shorter range. Best for when phone is on desk nearby. */
    LOW_POWER,
    /** Balanced detection range and battery. Default mode. */
    BALANCED,
    /** Maximum range, higher battery usage. For hands-free across room. */
    HIGH_SENSITIVITY
}
...
class HighSensitivitySpeechService(
    ...
    private val powerProfile: WakeWordPowerProfile = WakeWordPowerProfile.BALANCED,
    private val enableVerificationBuffer: Boolean = false  // Disabled by default to save memory
) {
    companion object {
        ...
        // Optimized gain values per power profile
        private const val GAIN_LOW_POWER = 1.5f       // Minimal amplification
        private const val GAIN_BALANCED = 2.5f        // Default
        private const val GAIN_HIGH_SENSITIVITY = 4.0f // Maximum range

        // Buffer multiplier per profile (higher = better capture, more memory)
        private const val BUFFER_MULT_LOW_POWER = 2
        private const val BUFFER_MULT_BALANCED = 4
        private const val BUFFER_MULT_HIGH_SENSITIVITY = 6
    }

    // Effective gain based on profile (overrides constructor param if profile specified)
    private val effectiveGain: Float = when (powerProfile) {
        WakeWordPowerProfile.LOW_POWER -> GAIN_LOW_POWER
        WakeWordPowerProfile.BALANCED -> gainMultiplier  // Use provided value
        WakeWordPowerProfile.HIGH_SENSITIVITY -> GAIN_HIGH_SENSITIVITY
    }

    private val bufferMultiplier: Int = when (powerProfile) {
        WakeWordPowerProfile.LOW_POWER -> BUFFER_MULT_LOW_POWER
        WakeWordPowerProfile.BALANCED -> BUFFER_MULT_BALANCED
        WakeWordPowerProfile.HIGH_SENSITIVITY -> BUFFER_MULT_HIGH_SENSITIVITY
    }
    ...
    // Rolling buffer for speaker verification - only allocated if enabled
    private val verificationBufferSize = if (enableVerificationBuffer) (sampleRate * VERIFICATION_BUFFER_SECONDS).toInt() else 0
    private val verificationBuffer: ShortArray? = if (enableVerificationBuffer) ShortArray(verificationBufferSize) else null
    private var bufferWriteIndex = 0
    private val bufferLock = Any()

    // Pre-allocated byte buffer for audio conversion (reduces GC pressure)
    private var cachedByteBuffer: ByteArray? = null
    ...
    fun startListening(listener: RecognitionListener) {
        ...
        try {
            ...
            // Use buffer size based on power profile
            val actualBufferSize = bufferSize * bufferMultiplier
            ...
            Log.i(TAG, "Started with ${effectiveGain}x gain (${powerProfile.name} mode)")
        ...
        try {
            while (isRunning) {
                ...
                if (readCount > 0) {
                    ...
                    // Only store in verification buffer if enabled (saves CPU cycles)
                    if (enableVerificationBuffer && verificationBuffer != null) {
                    ...
                    // Convert to byte array for Vosk (reuse buffer to reduce GC)
     ...
     * OPTIMIZED: Uses effectiveGain based on power profile.
     */
    private fun applyGain(input: ShortArray, output: ShortArray, count: Int) {
        // Skip processing if gain is ~1.0 (LOW_POWER optimization)
        if (effectiveGain <= 1.1f) {
        ...
        for (i in 0 until count) {
            ...
            val amplified = (input[i] * effectiveGain).toInt()
     ...
     * OPTIMIZED: Reuses cached buffer to reduce GC pressure.
     */
    private fun shortsToBytes(shorts: ShortArray, count: Int): ByteArray {
        val requiredSize = count * 2
        // Reuse buffer if large enough, otherwise allocate new one
        val bytes = if (cachedByteBuffer != null && cachedByteBuffer!!.size >= requiredSize) {
            cachedByteBuffer!!
        } else {
            ByteArray(requiredSize).also { cachedByteBuffer = it }
        }
     ...
     * Returns empty array if verification buffer is disabled.
     */
    fun getRecentAudioSamples(): ShortArray {
        if (!enableVerificationBuffer || verificationBuffer == null) {
            Log.w(TAG, "Verification buffer disabled - returning empty samples")
            return ShortArray(0)
        }
    ...
    fun clearVerificationBuffer() {
        if (!enableVerificationBuffer || verificationBuffer == null) return
        synchronized(bufferLock) {
    ...
    fun stop() {
        ...
        // CRITICAL FIX: Stop AudioRecord BEFORE joining thread to unblock the read loop
        // This prevents UI thread freezing when switching modes
        try {
            audioRecord?.stop()
        } catch (e: Exception) {
