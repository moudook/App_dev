...
class ChatMessageMapperTest {
    ...
    fun `calculateDisplayPosition starts from 0 when streaming`() {
        val position =
            ChatMessageMapper.calculateDisplayPosition(
                messageId = "1",
    ...
    fun `getVisibleText returns substring based on position`() {
        val visible =
            ChatMessageMapper.getVisibleText(
                fullText = "Hello World",
                displayPosition = 5,
            )
    ...
    fun `updateMessageInState updates correct message`() {
        ...
        val newState =
            ChatMessageMapper.updateMessageInState(
    ...
    fun `markMessageCompleteInState sets isStreaming to false`() {
        ...
        val newState =
            ChatMessageMapper.markMessageCompleteInState(
                currentState = initialState,
                messageId = "1",
            )
