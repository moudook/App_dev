...
class CalendarFeatureTest {
    ...
    fun `create event with valid data succeeds`() = runTest {
        val eventData = TestBuilders.calendarEvent {
            title = "Team Meeting"
    ...
    @Test
    fun `create event with end before start fails`() = runTest {
        val eventData = TestBuilders.calendarEvent {
            title = "Invalid Event"
    ...
    @Test
    fun `update event preserves metadata`() = runTest {
        val originalEvent = TestBuilders.calendarEvent {
            title = "Original"
            isRecurring = true
            reminderMinutes = 15
        }
    ...
    @Test
    fun `delete event removes from repository`() = runTest {
    ...
    @Test
    fun `filter upcoming events returns future events only`() = runTest {
    ...
    @Test
    fun `filter past events returns completed events`() = runTest {
        val now = System.currentTimeMillis()
        val events = listOf(
    ...
    @Test
    fun `filter recurring events returns only recurring`() = runTest {
        val events = listOf(
    ...
    @Test
    fun `search events by title returns matches`() = runTest {
        val events = listOf(
    ...
    @Test
    fun `search events by description returns matches`() = runTest {
        val events = listOf(
    ...
    @Test
    fun `private event is marked as private`() = runTest {
    ...
    @Test
    fun `private events excluded from AI context`() = runTest {
    ...
    @Test
    fun `event with reminder has valid minutes`() = runTest {
        val event = TestBuilders.calendarEvent {
            reminderMinutes = 30
        }
    ...
    @Test
    fun `event without reminder has null or zero`() = runTest {
        val event = TestBuilders.calendarEvent {
    ...
    @Test
    fun `event with google ID is synced`() = runTest {
        val event = TestBuilders.calendarEvent {
    ...
    @Test
    fun `event without google ID is not synced`() = runTest {
        val event = TestBuilders.calendarEvent {
            googleEventId = null
        }
    ...
    @Test
    fun `all-day event has correct flag`() = runTest {
        val event = TestBuilders.calendarEvent {
            isAllDay = true
        }
    ...
    @Test
    fun `all-day event spans full day`() = runTest {
        val event = TestBuilders.calendarEvent {
            isAllDay = true
        }
    ...
    @Test
    fun `event with location has valid location`() = runTest {
        val event = TestBuilders.calendarEvent {
    ...
    @Test
    fun `event without location has null location`() = runTest {
        val event = TestBuilders.calendarEvent {
            location = null
        }
    ...
    @Test
    fun `sort events by start time ascending`() = runTest {
        val now = System.currentTimeMillis()
        val events = listOf(
            TestBuilders.calendarEvent { startTime = now + 172800000 },
            TestBuilders.calendarEvent { startTime = now + 86400000 },
    ...
    @Test
    fun `detect overlapping events`() = runTest {
        val event1 = TestBuilders.calendarEvent {
            startTime = 1000
            endTime = 2000
        }
        val event2 = TestBuilders.calendarEvent {
            startTime = 1500
            endTime = 2500
        }
    ...
    @Test
    fun `no conflict for non-overlapping events`() = runTest {
        val event1 = TestBuilders.calendarEvent {
            startTime = 1000
            endTime = 2000
        }
