...
class ChatMessageUseCasesTest {
    ...
    @Test
    fun `SendMessageUseCase throws when content is blank and no attachments`() =
        runTest {
    ...
    @Test
    fun `SendMessageUseCase creates and saves user message`() =
        runTest {
    ...
    @Test
    fun `SendMessageUseCase creates message with attachments`() =
        runTest {
            val sessionId = "session1"
    ...
    @Test
    fun `UpdateMessageUseCase updates message content`() =
        runTest {
            val sessionId = "session1"
            val messageId = "msg1"
    ...
    @Test
    fun `UpdateMessageUseCase updates thinking when provided`() =
        runTest {
            val sessionId = "session1"
            val messageId = "msg1"
            val content = "Content"
            ...
            verify(chatRepository).updateMessage(
                eq(sessionId),
                eq(messageId),
                any { message ->
                    assertEquals(content, message.content)
    ...
    @Test
    fun `UpdateMessageUseCase sets isStreaming flag`() =
        runTest {
            val sessionId = "session1"
            val messageId = "msg1"
    ...
    @Test
    fun `GetMessagesUseCase returns flow of messages`() =
        runTest {
            val sessionId = "session1"
            val messages =
                listOf(
    ...
    @Test
    fun `GetMessagesUseCase getAllMessages returns list`() =
        runTest {
            val sessionId = "session1"
            val messages =
                listOf(
    ...
    @Test
    fun `ClearMessagesUseCase clears messages for session`() =
        runTest {
            val sessionId = "session1"
    ...
    @Test
    fun `DeleteMessageUseCase deletes specific message`() =
        runTest {
            val sessionId = "session1"
            val messageId = "msg1"
