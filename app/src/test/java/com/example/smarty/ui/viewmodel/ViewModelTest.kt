...
class ViewModelTest {
    ...
    @Test
    fun `load notes updates state`() = runTest {
        val notes = listOf(
    ...
    @Test
    fun `filter notes by category updates state`() = runTest {
        val allNotes = listOf(
    ...
    @Test
    fun `search notes updates state`() = runTest {
        val notes = listOf(
    ...
    @Test
    fun `create note adds to list`() = runTest {
    ...
    @Test
    fun `load events updates state`() = runTest {
        val events = listOf(
    ...
    @Test
    fun `filter upcoming events updates state`() = runTest {
    ...
    @Test
    fun `create event adds to list`() = runTest {
    ...
    @Test
    fun `delete event removes from list`() = runTest {
    ...
    @Test
    fun `load messages updates state`() = runTest {
        val messages = listOf(
    ...
    @Test
    fun `send message adds to list`() = runTest {
    ...
    @Test
    fun `clear chat resets state`() = runTest {
    ...
    @Test
    fun `error state set on chat failure`() = runTest {
    ...
    @Test
    fun `loading state updated correctly`() = runTest {
...
class NoteBuilder {
    ...
    fun archived() { isArchived = true }
    fun pinned() { isPinned = true }
...
class NotesViewModel(private val useCases: NoteUseCases) {
    ...
    suspend fun loadNotes() { state.value = state.value.copy(isLoading = true, notes = useCases.getAllNotes()) }
    suspend fun filterByCategory(category: String) { state.value = state.value.copy(isLoading = true) }
    suspend fun searchNotes(query: String) { state.value = state.value.copy(isLoading = true, notes = useCases.searchNotes(query)) }
    suspend fun createNote(title: String, content: String) { useCases.createNote(title, content); state.value = state.value.copy(isLoading = false) }
    suspend fun deleteNote(id: String) { useCases.deleteNote(id); state.value = state.value.copy(isLoading = false) }
}

class CalendarViewModel(private val useCases: CalendarUseCases) {
    val state = MutableStateFlow(CalendarState())
    suspend fun loadEvents() { state.value = state.value.copy(isLoading = true, events = useCases.getAllEvents()) }
    suspend fun loadUpcomingEvents() { state.value = state.value.copy(isLoading = true, events = useCases.getUpcomingEvents()) }
    suspend fun createEvent(title: String, startTime: Long, endTime: Long) { useCases.createEvent(title, startTime, endTime); state.value = state.value.copy(isLoading = false) }
    suspend fun deleteEvent(id: String) { useCases.deleteEvent(id); state.value = state.value.copy(isLoading = false) }
}

class ChatViewModel(private val useCases: ChatUseCases) {
    val state = MutableStateFlow(ChatState())
    suspend fun loadMessages(sessionId: String) { state.value = state.value.copy(isLoading = true, messages = useCases.getMessages(sessionId)) }
    suspend fun sendMessage(sessionId: String, content: String) { useCases.sendMessage(sessionId, content); state.value = state.value.copy(isLoading = false) }
    fun clearChat() { state.value = state.value.copy(messages = emptyList()) }
}
