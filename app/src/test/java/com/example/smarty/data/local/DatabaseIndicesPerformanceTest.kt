...
class DatabaseIndicesPerformanceTest {
    ...
    fun setup() {
        ...
        database = Room.inMemoryDatabaseBuilder(context, SmartyDatabase::class.java)
    ...
    @Test
    fun verifyChatMessagesIndicesExist() = runBlocking {
        ...
        assertTrue("Missing composite index sessionId+role", 
            indexNames.any { it.contains("sessionId") && it.contains("role") })
        assertTrue("Missing composite index sessionId+role+timestamp",
            indexNames.any { it.contains("sessionId") && it.contains("role") && it.contains("timestamp") })
    }

    @Test
    fun verifyChatSessionsIndicesExist() = runBlocking {
        val indices = database.query("""
            SELECT name FROM sqlite_master 
        ...
        assertTrue("Missing composite index isActive+updatedAt",
    ...
    @Test
    fun sessionMessageLookup_performance() = runBlocking {
        ...
        assertTrue("Average query time too slow: ${avgTimeMs}ms (expected <10ms)", 
            avgTimeMs < 10)
    ...
    @Test
    fun roleFilteringPerformance() = runBlocking {
        ...
        assertTrue("Role filter query too slow: ${avgTimeMs}ms (expected <5ms)",
            avgTimeMs < 5)
    ...
    @Test
    fun activeSessionLookup_performance() = runBlocking {
        ...
        assertTrue("Active session lookup too slow: ${avgTimeMs}ms (expected <1ms)",
            avgTimeMs < 1)
    ...
    @Test
    fun recentSessionsQuery_performance() = runBlocking {
        ...
        assertTrue("Recent sessions query too slow: ${avgTimeMs}ms (expected <5ms)",
            avgTimeMs < 5)
    ...
    @Test
    fun getMessagesByRole_returnsCorrectMessages() = runBlocking {
        // Setup
        val session = ChatSession(
            id = UUID.randomUUID().toString(),
            title = "Role Test"
    ...
    @Test
    fun getActiveSession_returnsActiveSession() = runBlocking {
        // Setup
        val activeSession = ChatSession(
            id = "active-123",
            title = "Active Session",
    ...
    @Test
    fun getRecentSessions_returnsOrderedSessions() = runBlocking {
        // Setup: Create sessions with different update times
        val oldSession = ChatSession(
            id = "old-123",
            title = "Old Session",
