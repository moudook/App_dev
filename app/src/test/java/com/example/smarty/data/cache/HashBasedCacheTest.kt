...
class HashBasedCacheTest {
    ...
    @Test
    fun `action query - play keyword bypasses cache`() = runTest {
    ...
    @Test
    fun `non-action query - general question is cached`() = runTest {
    ...
    @Test
    fun `non-action query - factual query is cached`() = runTest {
    ...
    @Test
    fun `non-action query - help query is cached`() = runTest {
    ...
    @Test
    fun `non-action query - definition query is cached`() = runTest {
    ...
    @Test
    fun `non-action query - weather query is cached`() = runTest {
    ...
    @Test
    fun `get returns null for blank query`() = runTest {
        assertNull(cache.get(""))
        assertNull(cache.get("   "))
    }

    @Test
    fun `put does not store blank query`() = runTest {
        cache.put("", "response")
    ...
    @Test
    fun `cache stores and retrieves responses`() = runTest {
    ...
    @Test
    fun `cache updates statistics on hit`() = runTest {
    ...
    @Test
    fun `cache updates statistics on miss`() = runTest {
    ...
    @Test
    fun `normalized queries match - case insensitive`() = runTest {
    ...
    @Test
    fun `normalized queries match - whitespace`() = runTest {
        cache.put("hello   world", "response")
        val result = cache.get("hello world")
        assertEquals("response", result)
    }
    ...
    @Test
    fun `normalized queries match - trailing punctuation`() = runTest {
        cache.put("hello world!", "response")
        val result = cache.get("hello world")
        assertEquals("response", result)
    }
    ...
    @Test
    fun `normalized queries match - combined normalization`() = runTest {
        cache.put("  HELLO   World?!  ", "response")
        val result = cache.get("hello world")
        assertEquals("response", result)
    }
    ...
    @Test
    fun `clear resets statistics`() = runTest {
    ...
    @Test
    fun `invalidateNoteDataEntries removes only note data entries`() = runTest {
    ...
    @Test
    fun `cache evicts when at capacity`() = runTest {
    ...
    @Test
    fun `cache evicts least used entries`() = runTest {
        // Fill cache
        for (i in 1..10) {
            cache.put("query$i", "response$i")
        }
    ...
    @Test
    fun `getStats returns correct hit rate`() = runTest {
        cache.put("test", "response")
        cache.get("test") // hit
        cache.get("test") // hit
        cache.get("nonexistent") // miss
    ...
    @Test
    fun `getStats returns zero hit rate when empty`() = runTest {
        val stats = cache.getStats()
    ...
    @Test
    fun `action keyword at end of query bypasses cache`() = runTest {
        cache.put("something about notes", "response")
        assertNull(cache.get("something about notes"))
    }

    @Test
    fun `action keyword in middle of query bypasses cache`() = runTest {
        cache.put("can you find something for me", "response")
        assertNull(cache.get("can you find something for me"))
    }

    @Test
    fun `mixed case action keywords bypass cache`() = runTest {
        cache.put("PLAY my Music", "response")
        assertNull(cache.get("PLAY my Music"))
    }

    @Test
    fun `query with only whitespace returns null`() = runTest {
        assertNull(cache.get("   \t\n  "))
    }

    @Test
    fun `very long query is handled correctly`() = runTest {
