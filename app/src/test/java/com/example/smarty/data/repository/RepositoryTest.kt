...
class RepositoryTest {
    ...
    @Test
    fun `create note inserts to database`() = runTest {
    ...
    @Test
    fun `get all notes returns list`() = runTest {
        val notes = listOf(
    ...
    @Test
    fun `update note modifies database`() = runTest {
        val note = TestBuilders.note {
            id = "note-123"
            title = "Updated"
        }
    ...
    @Test
    fun `delete note removes from database`() = runTest {
        val noteId = "note-123"
    ...
    @Test
    fun `create event inserts to database`() = runTest {
    ...
    @Test
    fun `get event by id returns event`() = runTest {
    ...
    @Test
    fun `update event modifies database`() = runTest {
        val event = TestBuilders.calendarEvent {
            id = "event-123"
            title = "Updated Event"
        }
    ...
    @Test
    fun `save message inserts to database`() = runTest {
        val message = TestBuilders.chatMessage {
            id = "msg-123"
    ...
    @Test
    fun `get messages by session returns list`() = runTest {
        val messages = listOf(
    ...
    @Test
    fun `delete session removes messages`() = runTest {
    ...
    @Test
    fun `get recent sessions returns ordered list`() = runTest {
        val sessions = listOf(
...
class CalendarRepository(private val dao: CalendarDao) {
    suspend fun create(event: com.example.smarty.common.src.commonMain.kotlin.com.example.smarty.data.model.CalendarEvent) = dao.insert(event)
    suspend fun getById(id: String) = dao.getById(id)
    suspend fun getUpcoming() = dao.getUpcoming()
    suspend fun update(event: com.example.smarty.common.src.commonMain.kotlin.com.example.smarty.data.model.CalendarEvent) = dao.update(event)
    suspend fun delete(id: String) = dao.delete(id)
}

class ChatRepository(private val dao: ChatDao) {
    suspend fun saveMessage(message: com.example.smarty.common.src.commonMain.kotlin.com.example.smarty.data.model.ChatMessage) = dao.insert(message)
...
interface ChatDao {
    ...
    suspend fun getBySession(sessionId: String): List<com.example.smarty.common.src.commonMain.kotlin.com.example.smarty.data.model.ChatMessage>
