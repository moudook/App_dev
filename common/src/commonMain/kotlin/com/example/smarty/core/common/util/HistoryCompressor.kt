 ...
 *
 * Strategy:
 * - Keep last N exchanges (user+smarty pairs) verbatim
 * - Extract facts from older exchanges into structured history
...
class HistoryCompressor(
    ...
    fun compress(
        ...
        // Calculate threshold: 2 messages per exchange (user + smarty)
    ...
    /**
     * Extract structured facts from messages for long-term history.
    ...
    private fun extractFacts(messages: List<ChatMessage>): List<ExtractedFact> {
        ...
        messages.forEach { message ->
            ...
            // Extract task outcomes from smarty actions
            if (message.isSmarty && message.executedActions.isNotEmpty()) {
    ...
    private fun summarize(
        ...
        // Group into exchanges (user + smarty pairs)
        val exchanges = messages.chunked(2)

        exchanges.forEach { exchange ->
            val userMsg = exchange.find { it.isUser }
            val smartyMsg = exchange.find { it.isSmarty }
            ...
            // Extract smarty action/response
            smartyMsg?.let {
                ...
                if (it.executedActions.isNotEmpty()) {
                    ...
                    val excerpt = extractKeyPoints(it.content)
                    if (excerpt.isNotBlank()) {
                        summaryParts.add("Smarty: $excerpt")
                    }
                }
            }
        }
